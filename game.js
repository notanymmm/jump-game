document.addEventListener('DOMContentLoaded', () => {
  try {
    ...
    let gameInterval, obsInterval, timerInterval, difficultyInterval, colorSwapInterval;
    ...
    let moveReverseEnabled = false;
    let isColorSwapped = false;
    let touchStart = null;
    ...
    function resetPlayer() {
      try {
        ...
        player.style.background = 'red'; // Đặt lại màu ban đầu
        document.querySelectorAll('.obstacle').forEach(obstacle => {
          obstacle.style.background = 'green'; // Đặt lại màu vật cản
        });
        isColorSwapped = false;
        moveReverseEnabled = false;
    ...
    function showNotification(message) {
      try {
        ...
        console.error('Lỗi trong showNotification:', error);
      }
    }
    ...
    function swapColors() {
      try {
        ...
        console.error('Lỗi trong swapColors:', error);
      }
    }
    ...
    joystickArea.addEventListener('touchmove', e => {
      ...
      try {
        ...
        if (moveReverseEnabled) {
          dirs.x = -dirs.x; // Đảo ngược hướng x cho joystick
          dirs.y = -dirs.y; // Đảo ngược hướng y cho joystick
        }
      } catch (error) {
    ...
    function spawnObstacle() {
      try {
        ...
        obstacle.style.background = isColorSwapped ? 'red' : 'green'; // Áp dụng màu đúng khi tạo vật cản
      ...
      try {
        ...
        timerInterval = setInterval(() => {
          if (gameRunning) {
            ...
            if (survivalTime >= 30 && !diagonalEnabled) {
              diagonalEnabled = true;
            }
            if (survivalTime >= 50 && !reverseEnabled) {
              reverseEnabled = true;
            }
            if (survivalTime >= 70 && !colorSwapEnabled) {
              colorSwapEnabled = true;
              swapColors();
              colorSwapInterval = setInterval(swapColors, 500); // Nhấp nháy mỗi 0.5 giây
            }
            if (survivalTime >= 87 && !moveReverseEnabled) { // Hiển thị thông báo trước 3 giây
              showNotification('Đảo ngược nào');
            }
            if (survivalTime >= 90 && !moveReverseEnabled) {
              moveReverseEnabled = true; // Kích hoạt ngược di chuyển
            }
          }
        }, 100);
    ...
    function endGame() {
      try {
        ...
        clearInterval(colorSwapInterval); // Dừng nhấp nháy khi kết thúc
